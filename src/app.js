...
httpsServer.listen(config.listenPort, () => {
  ...
  console.log(config.mediasoup. webRtcTransport.listenIps)
})
...
;(async () => {
  ...
  newPublicIp=response.body.replace(/[\n\r]+/g, '');
  console.log(newPublicIp);
  config.mediasoup.webRtcTransport.listenIps=[
    {
      ip: '0.0.0.0',
      announcedIp: newPublicIp
    }
  ];
...
io.on('connection', (socket) => {
  socket.on('createRoom', async ({ room_id }, callback) => {
    if (roomList.has(room_id)) {
      ...
      console.log('Created room', { room_id: room_id,publicIp: newPublicIp })
  ...
  socket.on('join', ({ room_id, name }, cb) => {
    ...
    cb(roomList.get(room_id).toJson(newPublicIp))
  })
